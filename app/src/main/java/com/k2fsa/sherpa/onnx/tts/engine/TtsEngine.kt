...
object TtsEngine {
    ...
    val numThreadsState: MutableState<Int> = mutableIntStateOf(6)
    ...
    init {
        ...
        //
        // For VITS -- begin
        modelName = "vits-zh-hf-fanchen-Qwen_1000.onnx"
        // For VITS -- end
    ...
    private fun initTts(context: Context) {
        ...
        val selectedModel = pref.getModelName()
        
        // Determine if it's an asset model (built-in) or a custom model
        isAssetModel = selectedModel == "paimeng"
        assets = if (isAssetModel) context.assets else null

        if (isAssetModel) {
            // Built-in Paimon logic
            modelDir = "paimeng"
            modelName = "vits-zh-hf-fanchen-Qwen_1000.onnx"
            lexicon = "lexicon.txt"
            lang = "zho"
            ...
            // Custom model logic
            val baseDir = File(context.getExternalFilesDir(null), "models/$selectedModel")
            if (!baseDir.exists()) {
                Log.e(TAG, "Model directory not found: ${baseDir.absolutePath}")
                return
            }

            modelDir = baseDir.absolutePath
            
            // Search for the first .onnx file in the directory
            val onnxFile = baseDir.listFiles()?.find { it.name.endsWith(".onnx") }
            if (onnxFile == null) {
                Log.e(TAG, "No .onnx file found in ${baseDir.absolutePath}")
                return
            }
            modelName = onnxFile.name
            Log.i(TAG, "Found model file: $modelName")

            lexicon = if (File(baseDir, "lexicon.txt").exists()) "lexicon.txt" else ""
            
            // Try to read lang from a 'lang.txt' file if it exists, otherwise default to zho
            val langFile = File(baseDir, "lang.txt")
            lang = if (langFile.exists()) langFile.readText().trim() else "zho"

            isKitten = false
            dataDir = if (File(baseDir, "espeak-ng-data").exists()) "$modelDir/espeak-ng-data" else null
        }

        try {
            currentProvider = "nnapi" // You can change this to cpu to test the difference
            ...
            Log.e(TAG, "Failed to create TTS: $e")
            tts = null
        }
    }
